...
def process_dbscan():
    ...
    # Redimensionnement pour performance (DBSCAN est lent)
    img_small = img.copy()
    ...
    for label in unique_labels:
        if label == -1:
            # Le bruit est coloré en noir
            new_pixels[labels == label] = [0, 0, 0]
        else:
            # Calcul de la couleur moyenne pour la classe
            mean_color = pixels[labels == label].mean(axis=0).astype('uint8').tolist()
            new_pixels[labels == label] = mean_color
            palette.append(mean_color)

    stats = {
        "k_found": len(palette),
        "noise_pts": list(labels).count(-1),
        "noise_pct": round((list(labels).count(-1)/len(labels))*100, 1),
        "palette": palette,
        "eps": eps,
        "ms": ms
    }
    ...
    result_img = Image.fromarray(new_pixels.reshape(img_np.shape)).resize(img.size, Image.NEAREST)
    res_name = f"dbscan_{eps}_{photo}"
    result_img.save(os.path.join(dossier, res_name))
    
    images = [f for f in os.listdir(dossier) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    return render_template("dbscan.html", images=images, dossier=dossier, photo_finale=res_name, stats=stats)
