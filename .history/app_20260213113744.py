...
# Style CSS centralisé pour l'accueil
style = """
<style>
    @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;500&display=swap');
    body { font-family: 'Poppins', sans-serif; background-color: #f0f2f5; color: #1c1e21; display: flex; flex-direction: column; align-items: center; min-height: 100vh; margin: 0; padding: 20px; }
...
def run_clustering(method):
    ...
    # Redimensionnement selon l'algo
    limit = 150 if method == "dbscan" else (100 if method == "hc" else 400)
    img_small = img.copy()
    img_small.thumbnail((limit, limit))
    img_np = np.array(img_small)
    pixels = img_np.reshape(-1, 3)
    ...
    if method == "kmeans":
        ...
        eps = float(request.form.get('eps', 5))
        ms = int(request.form.get('min_samples', 10))
        ...
        stats = {"k_found": n_clusters, "noise_pts": n_noise, "noise_pct": round((n_noise/len(labels))*100, 1), "eps": eps, "ms": ms}

        new_pixels = np.zeros_like(pixels)
        for label in unique_labels:
            if label == -1: new_pixels[labels == label] = [0, 0, 0]
            else: new_pixels[labels == label] = pixels[labels == label].mean(axis=0).astype('uint8')
        suffix = f"dbscan_{eps}"
...
@app.route('/view_clusters', methods=['POST'])
def view_clusters():
    dossier = request.form.get('dossier')
    photo = request.form.get('photo')
    method = request.form.get('method')
    
    img = Image.open(os.path.join(dossier, photo)).convert('RGB')
    img.thumbnail((150, 150))
    img_np = np.array(img)
    pixels = img_np.reshape(-1, 3)

    if method == "dbscan":
        model = DBSCAN(eps=float(request.form.get('eps', 5)), min_samples=int(request.form.get('min_samples', 10)))
    else:
        k = int(request.form.get('k', 5))
    ...
    canvas = Image.new('RGB', (grid_size * cell_w, grid_size * cell_h), (30, 30, 30))

    for i, label in enumerate(unique_labels):
        cluster_pix = np.zeros_like(pixels)
        cluster_pix[labels == label] = pixels[labels == label]
        c_img = Image.fromarray(cluster_pix.reshape(img_np.shape))
        canvas.paste(c_img, ((i % grid_size) * cell_w, (i // grid_size) * cell_h))

    grid_path = os.path.join(dossier, "decomp_" + photo)
    canvas.save(grid_path)
    return send_from_directory(dossier, "decomp_" + photo)
