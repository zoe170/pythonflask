...
@app.route('/about')
def about():
    return style + '<h1>À propos</h1><p>Comparaison sans sauvegarde locale.</p><a href="/">Retour</a>'
...
def handle_gallery(template_name):
    images = []
    dossier_choisi = request.form.get('chemin_dossier', '')
    # On récupère l'image d'origine pour l'affichage
    photo_selectionnee = request.form.get('menu_photos', '')
    if dossier_choisi and os.path.exists(dossier_choisi):
        # On ne liste que les fichiers originaux (on évite les anciens fichiers générés s'il en reste)
        images = [f for f in os.listdir(dossier_choisi) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
...
def run_clustering(method):
    ...
    # 1. Chargement
    path_input = os.path.join(dossier, photo)
    img_orig = Image.open(path_input).convert('RGB')
    
    # 2. Redimensionnement pour performance
    limit = 100 if method == "hc" else 400
    ...
    # 5. Reconstruction et redimensionnement à la taille d'origine
    new_pixels = colors[labels]
    new_img_np = new_pixels.reshape(img_np.shape)
    result_img = Image.fromarray(new_img_np)
    result_img = result_img.resize(img_orig.size, Image.NEAREST)

    # 6. ENCODAGE EN BASE64 (Pas de sauvegarde sur disque)
    buf = io.BytesIO()
    result_img.save(buf, format='PNG')
    # On transforme l'image en texte pour l'envoyer au HTML
    img_b64 = base64.b64encode(buf.getvalue()).decode('utf-8')
    img_data = f"data:image/png;base64,{img_b64}"
    
    # 7. Retour vers la page
    images = [f for f in os.listdir(dossier) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    template = "photo.html" if method == "kmeans" else "hierarchical.html"
    
    # On passe 'img_data' (le flux mémoire) au lieu d'un nom de fichier
    return render_template(template, images=images, dossier=dossier, photo_finale=photo, result_image=img_data)
